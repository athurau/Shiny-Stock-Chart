...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(color = 'cadetblue4',
                 ...
                 textInput("symb", label = h4("Input a Valid Stock Ticker"), value = "AAPL"),
                 textInput("symb2", label = h4("Input a Valid Stock Ticker"), value = "RTN"),
                 textInput("symb3", label = h4("Input a Valid Stock Ticker"), value = "GE"),
...
server <- function(input, output) {
  dataInput <- reactive({
    ...
    prices3 <- getSymbols(input$symb3, from = input$date[1], to = input$date[2], auto.assign = FALSE)
    market <- getSymbols("^GSPC", from = input$date[1], to = input$date[2], auto.assign = FALSE)
    t <- getSymbols.FRED("TB1YR", auto.assign = FALSE)
    dataPrices <- merge.xts(Ad(prices), Ad(market), Ad(prices2), Ad(prices3), join = "inner")
  ...
  output$plot <- renderPlot({
    ...
    qplot(theData$x[,2], theData$x[,1], ylab = "Asset Returns", xlab = "Market Returns") +    
      geom_abline(intercept =theData$y$coef[1], slope = theData$y$coef[2], color = "red") +
      geom_abline(intercept =theData$reg2$coef[1], slope = theData$reg2$coef[2], color = "blue") +
      geom_abline(intercept =theData$reg3$coef[1], slope = theData$reg3$coef[2], color = "grey23") +
  ...
  output$SML <- renderPlot({
    ...
    qplot(theData$y$coef[2], theData$z, ylim = c(0, .15), xlim = c(0, 2), ylab = "Expected Return", xlab = "Beta") +
      geom_abline(intercept =theData$rF, slope = ((theData$z - theData$rF)/theData$y$coef[3])) +
